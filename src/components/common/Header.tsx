...
const NavLinks: React.FC<NavLinkProps> = ({ path, url, onClick }) => {
  return (
    <NavLink to={path} className="text-gray-800 hover:text-blue-600 transition-colors duration-300" onClick={onClick}>
      {url}
    </NavLink>
  )
}
...
const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, toggleMenu }) => (
  ...
  >
    {NavLists.map((list) => (
      <NavLinks key={list.id} path={list.path} url={list.url} onClick={toggleMenu} />
    ))}
    {/* <AccountButtons /> */}
  </nav>
);
